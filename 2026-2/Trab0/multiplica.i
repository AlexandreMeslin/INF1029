...
# 51 "multiplica.c" 3 4
               stderr
# 51 "multiplica.c"
                     , "[ERROR %d] Erro ao alocar memória para a matriz A\n", 51);
        exit(1);
    }
    le_matriz(argv[1], &a);
...
# 62 "multiplica.c" 3 4
               stderr
# 62 "multiplica.c"
                     , "[ERROR %d] Erro ao alocar memória para a matriz B\n", 62);
        exit(2);
    }
    le_matriz(argv[4], &b);
...
# 73 "multiplica.c" 3 4
               stderr
# 73 "multiplica.c"
                     , "[ERROR %d] Erro ao alocar memória para a matriz C\n", 73);
        exit(3);
    }
    ...
    for(int i=0; i<c.n_linhas; i++) {
        for(int j=0; j<c.n_colunas; j++) {
            ...
            *(c.p_matriz + i * c.n_colunas + j) = 0;
            for(int k=0; k<a.n_colunas; k++) {

                *(c.p_matriz + i * c.n_colunas + j) += *(a.p_matriz + i * a.n_colunas + k) *
                                                       *(b.p_matriz + k * b.n_colunas + j);
                (*((c).p_matriz + (i) * (c).n_colunas + (j))) += (*((a).p_matriz + (i) * (a).n_colunas + (k))) * (*((b).p_matriz + (k) * (b).n_colunas + (j)));
            }
        }
    }
...
void le_matriz(char *nome, t_matriz *matriz) {
    ...
    if(!arq) {
        fprintf(
# 112 "multiplica.c" 3 4
               stderr
# 112 "multiplica.c"
                     , "[ERROR %d] Não foi possível abrir o arquivo %s para leitura\n",
            113, nome);
        exit(4);
    }
    ...
    if(n != matriz->n_linhas * matriz->n_colunas) {
        fprintf(
# 118 "multiplica.c" 3 4
               stderr
# 118 "multiplica.c"
                     , "[ERROR %d] Erro no tamanho da matriz %s (%d != %d)\n",
            119, nome, n, matriz->n_linhas * matriz->n_colunas);
        exit(5);
    }
    fclose(arq);
}
...
void salva_matriz(char *nome, t_matriz *matriz) {
    ...
    if(!arq) {
        fprintf(
# 140 "multiplica.c" 3 4
               stderr
# 140 "multiplica.c"
                     , "[ERROR %d] Não foi possível abrir o arquivo %s para escrita\n",
            141, nome);
        exit(6);
    }
    ...
    if(n != matriz->n_linhas * matriz->n_colunas) {
        fprintf(
# 146 "multiplica.c" 3 4
               stderr
# 146 "multiplica.c"
                     , "[ERROR %d] Erro ao escrever a matriz %s (%d != %d)\n",
            147, nome, n, matriz->n_linhas * matriz->n_colunas);
        exit(7);
    }
    fclose(arq);
}
