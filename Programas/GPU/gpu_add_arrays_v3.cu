...
#define DATASET_SIZE (1024000 * 10)
...
__global__ void add(int n, float *d_x, float *d_y) {
    int nThreads = gridDim.x * blockDim.x;  // total de threads
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int qtdPos = (n + (nThreads-1)) / nThreads; // qtd de posições que essa thread vai somar
    int posInicial = index * qtdPos;            // posição inicial do vetor que essa thread vai somar
    for(int i=0; i<qtdPos; i++) {
        if (posInicial + i < n) {   // verifica se ainda está dentro do vetor ou se já acabou
            // ainda está dentro do vetor, então soma!
            d_y[posInicial + i] = d_x[posInicial + i] + d_y[posInicial + i];
        }
    }
}
